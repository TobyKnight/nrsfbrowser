({ // NOSONAR
    ...
    hookErrorEvents : function(component, helper) {
        window.onerror = function(error, url, line) {
            helper.logIt(component, '*** Mon: page error detected: '+ error+ ' URL:'+url+' L:'+line);
                ...
                if (activeUItabs.length > 0) {
                    ...
                    for (let i = 0; i < activeUItabs.length; i++) {
                        if (helper.isBoundedBy(component, helper, activeUItabs[i], mouseX, mouseY)) {
                            ...
                            helper.logIt(component, '*** Mon: You clicked ' + aPath);
    ...
    firePseudoLocationChange : function(component, helper, aPath) {
        helper.logIt(component, '*** Mon : about to fire network inspection initiated location change');
       
        try {
            let msg={newPath : aPath}; 
            helper.logIt(component, '*** Mon : custom path set to: ' + msg.newPath);
            ...
            parent.postMessage(msg,consoleDomain);
            helper.logIt(component, `*** Mon: sent psuedo location change event to ${consoleDomain}`);
