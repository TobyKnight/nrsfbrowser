({ // NOSONAR
	...
	doGetUIDetails : function(component, event, helper) {
        // setTimeout(function() {
            ...
            helper.logIt(component, '*** Plr: getting UI components and record ids, route is:' + standardisedRoute);
            if (standardisedRoute && standardisedRoute.search('/hover') != -1) {
                isHover = true;
                helper.logIt(component, '*** Plr: scraping hover component only');
            ...
            helper.logIt(component, docCopy);
                    ...
                    if(href!= undefined && !href.includes('javascript:void')) {
                        if (isHover && hoverElement) {
                            if (!aTags[i].closest('.previewContainer')) {
                                helper.logIt(component, '*** Plr: ignored <a> element outside hover component');
                                
                                continue;
                            } else {
                                helper.logIt(component, '*** Plr: found <a> element inside hover component');
