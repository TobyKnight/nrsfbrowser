({  // NOSONAR
	doInit : function(component, event, helper) {
       ...
       // component.set("v.UserLocation", component.get("v.UserLocation"));
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                if (component.get("v.agentEnabled") === true) {
                    ...
                    component.set("v.vfHostEnhancedFormat", response.getReturnValue().vfhost[2]);   
                    ...
                    {
                       if (userRecord.Country && userRecord.Country.length) {
                        location = userRecord.Country; // Use FederationId if available
                       } 
                    }
                    else if (userLocation == 'City')
                    {
                        if (userRecord.City && userRecord.City.length) {
                        location = userRecord.City; // Use FederationId if available
                        }
                        else if (userLocation == 'Department')
                    {
                        if (userRecord.Department && userRecord.Department.length) {
                            location = userRecord.Department; // Use FederationId if availabl
                        }
                    }
        }
                  ...
                  //  } 
                    ...
                    let userField = component.get("v.UserObject");
                    
                    
                    if (userField == 'Username')
                    {
                      if (userRecord.Username && userRecord.Username.length) {
                        federationId = userRecord.Username; // Use FederationId if available
                    }
                    }
                    else if (userField == 'Id')
                    {
                       if (userRecord.Id && userRecord.Id.length) {
                        federationId = userRecord.Id; // Use FederationId if available
                    } 
                    }
                    else if (userField == 'Name')
                    {
                        if (userRecord.Name && userRecord.Name.length) {
                        federationId = userRecord.Name; // Use FederationId if available
                    }
                    }    
                        
                    //// Set utility bar name
                    let bIsHidden = component.get("v.hiddenOn");
                    ...
                    $A.createComponent(
                        ...
                        function(netListener, status, errorMessage){
                            if (status === "SUCCESS") {
                                helper.logIt(component, '*** Mon: network event listener wired OK');
                                ...
                                if (!component.get("v.disableAutoLocationChangeDetection")) {
                                    ...
                                    helper.logIt(component, '*** Mon: auto detection of location changes (aura:locationChange) are wired and ENABLED');
                                    ...
                                    helper.logIt(component, '*** Mon: auto detection of URL location changes has been DISABLED by component properties');
                                }
                                ...
                                helper.logIt(component, '*** Mon: custom location change, network and spinner event handlers wired OK');
    ...
    handleHover : function (component, event, helper) {
        ...
        helper.logIt(component, '*** Mon *** : someone hovered, transcendental baby');
